...
"""
import keras
import keras.layers as tflayers
...
class ReLU6(tflayers.Layer):
    def __init__(self):
        ...
        self.relu6 = keras.layers.ReLU(max_value=6.0, name="ReLU6")
...
class Squeeze(tflayers.Layer):
    ...
    def call(self, input):
        x = keras.backend.squeeze(input, 1)
...
class GlobalAveragePooling2D(tflayers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.gap = keras.layers.AveragePooling2D(
            pool_size=pool_size,
...
# class ConvNormAct(keras.models.Model):
    def __init__(
            self,
            filters: int,
            kernel_size: int=3,
        ...
        _available_normalization = {
            ...
            # "bn":layers.batch_normalization(
                    ...
                    # name="BatchNorm",trainable=True,training=False)
            }
        self.norm = get_layer(norm_layer, _available_normalization, Identity())

        _available_activation = {
            "relu": keras.layers.ReLU(name="ReLU"),
            "relu6": ReLU6(),
            "hswish": HardSwish(),
            # "hsigmoid": HardSigmoid(),
            "softmax": keras.layers.Softmax(name="Softmax"),
        }
...
# class Bneck(keras.models.Model):
    def __init__(
            self,
            out_channels: int,
            exp_channels: int,
        ...
        self.bn = keras.layers.BatchNormalization(
                    momentum=0.9, epsilon= 1e-3,
                    name="depthwise/BatchNorm",trainable=True
                )
        if self.use_se:
        ...
        _available_activation = {
            "relu": keras.layers.ReLU(name=name+"/ReLU"),
...
# class SEBottleneck(keras.models.Model):
    def __init__(
            self,
            reduction: int=4,
            l2_reg: float=0.01,
    ...
    def build(self, input_shape):
        ...
        self.act = _HardSwish() #keras.activations.hard_sigmoid
        super().build(input_shape)
...
class LastStage(tflayers.Layer):
    ...
    def call(self, input):
        ...
        # x = self.dropout(x)
        # x = self.conv3(x)
        # x = keras.layers.Lambda(lambda x: nn.softmax(x))(x)
        # x = self.squeeze(x)
        return x
