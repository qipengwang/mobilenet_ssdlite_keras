...
from keras.layers import Input, Lambda, Activation, Conv2D, DepthwiseConv2D, Reshape, Concatenate, BatchNormalization, ReLU, Input
...
class ssdDownSampleBlock(keras.layers.Layer):
# class ssdDownSampleBlock(keras.models.Model):
    def __init__(self,conv_out_channel, sep_out_channel, id, model_type):
        ...
        self.bn1 = KL.BatchNormalization(epsilon=1e-3, momentum=0.999, name=self.name1+'/BatchNorm', trainable = True)
        ...
        self.bn2 = KL.BatchNormalization(epsilon=1e-3, momentum=0.999, name=self.name2+'/BatchNorm',trainable = True)
        ...
        self.bn3 = KL.BatchNormalization(epsilon=1e-3, momentum=0.999, name=self.name3+'/BatchNorm',trainable = True)
...
class MobileNetV3SSDLiteExtractor(keras.models.Model):
    def __init__(
            self,
...
class PredictBlock(keras.layers.Layer):
# class PredictBlock(keras.models.Model):
    def __init__(self,out_channel, sym, id):
        ...
        self.bn = BatchNormalization(epsilon=1e-3, momentum=0.999, name= name + '{}_depthwise/BatchNorm'.format(id),trainable = True)
...
class MobileNetV3SSDLite(keras.models.Model):
    def __init__(self,
            image_size,
            n_classes,
            mode='training',
    ...
    def call(self,inputs,training=False):
        x = Input(input_shape=(320, 320, 3))(inputs)
